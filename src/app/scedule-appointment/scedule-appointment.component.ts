...
import { Appointments, PatientsDetails, PhysicianSpecialist } from '../app.model';
...
export class SceduleAppointmentComponent implements OnInit {
  ...
  constructor(private registrationService: RegistrationService, private messageService: MessageService, private router: Router, public loginService: LoginService) {
  ...
  ngOnInit(): void {
    this.messageService.add({ severity: 'error', summary: 'Success', detail: 'Please fill the demographics before ' });
  ...
  bookedAppoitment() {
    ...
    if (Object.keys(this.userDetail.demographics).length > 0) {
      if (this.date10 != "" && this.notes != '' && this.type != '') {
        let patientDetails: any={notes:this.notes,schedule_time:this.date10,type: this.type,patient_id: this.userDetail._id};
        let id = this.selected_Doctor.username;
       
         this.registrationService.addAppointmentData(id, patientDetails).subscribe((data: any) => {
          console.log(data);

          if (data && data.success == true) {
            console.log(data);
            this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Scheduled Appointment' });
            this.sucess = true;
 ...
 // this.userDetail = JSON.parse(sessionStorage.getItem('userData') || '');
  ...
  filter_by_location() {
    if (this.location == "None") {
      ...
      this.Doctors_temp = this.Doctors.filter(item => item.location == this.location)
    }
  }
